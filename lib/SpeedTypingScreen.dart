  ...
  void _startTimer() {
    ...
    _timer = Timer.periodic(Duration(seconds: 1), (timer) {
      ...
      setState(() {
        if (_timeLeft > 0) {
          _timeLeft--;
          _progress = _timeLeft / (widget.minutes * 60);
        } else {
          _timer?.cancel();
  ...
  void _handleWordTyped(String value) {
    if (value.endsWith(' ')) {
      ...
      if (currentIndex < sampleWords.length) {
        bool isCorrect = typedWord == sampleWords[currentIndex];
        if (isCorrect) {
          wpm++;
        } else {
          errors++;
      ...
      setState(() {
        ...
        accuracy = totalTyped > 0 ? ((correctWords / totalTyped) * 100).toInt() : 0;
      });
    } else {
      setState(() {
        currentWord = value;
